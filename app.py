...
<style>
    ...
    .st-key-chart_panel {
        ...
        overflow-y: auto;
        overflow-x: hidden;
        padding: .45rem .35rem .8rem .15rem;
        border-top: 1px solid rgba(127,127,127,.16);
        scrollbar-width: thin;
    }

    .st-key-chart_panel::-webkit-scrollbar {width: 6px;}
    .st-key-chart_panel::-webkit-scrollbar-thumb {
...
DIRECTORATE_ORDER = ["PKKS", "KJS", "NAKER", "KUMKM", "KSE"]
DIRECTORATE_LABELS = {
    ...
    "KUMKM": "Koperasi-UMKM (KUMKM)",
    "KSE": "Kemandirian Sosial Ekonomi (KSE)",
...
def render_simulator(upload: dict) -> None:
    ...
    with graph_col:
        with st.container(key="chart_panel"):
            ...
            # Right-side navigation only: the left slider/scenario panel is unchanged.
            grouped_outputs = group_outputs_by_directorate(metadata.outputs)
            directorate_codes = ordered_directorates(grouped_outputs)

            filter_dir_col, filter_indicator_col = st.columns([1.05, 1.35])
            with filter_dir_col:
                selected_directorate = st.selectbox(
                    "Direktorat",
                    directorate_codes,
                    format_func=directorate_display,
                    key="graph_directorate",
                )

            directorate_outputs = grouped_outputs[selected_directorate]
            indicator_lookup = {item.name: item for item in directorate_outputs}
            indicator_options = ["__ALL__"] + list(indicator_lookup)

            with filter_indicator_col:
                selected_indicator = st.selectbox(
                    "Indikator",
                    indicator_options,
                    format_func=lambda value: (
                        f"Semua indikator ({len(directorate_outputs)})"
                        if value == "__ALL__"
                        else indicator_lookup[value].label
                    ),
                    key=f"graph_indicator_{selected_directorate}",
                )

            if selected_indicator == "__ALL__":
                selected_outputs = directorate_outputs
            else:
                selected_outputs = [indicator_lookup[selected_indicator]]

            if selected_directorate == "__UNGROUPED__":
                st.caption(
                    "Output ini belum memiliki tag direktorat. Tambahkan "
                    "`directorate=PKKS`, `KJS`, `NAKER`, `KUMKM`, atau `KSE` "
                    "pada Comment variabel @output di Vensim."
                )
