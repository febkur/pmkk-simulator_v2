...
<style>
    ...
    .landing-spacer { height: clamp(2.5rem, 9vh, 6rem); }

    .landing-title-card {
        width: min(760px, 92vw);
        border: 1.5px solid #83cdbd;
        border-radius: 26px;
        padding: 1.15rem 2rem 1.25rem;
        background: rgba(255,255,255,.82);
        box-shadow: 0 10px 34px rgba(30, 90, 76, .06);
        margin: 0 auto 2rem;
        text-align: center;
    }
    ...
    /* Turn Streamlit's uploader into a compact, centered upload card. */
    .st-key-landing_upload {
        width: min(360px, 86vw);
        margin: 0 auto .9rem;
    }

    .st-key-landing_upload [data-testid="stFileUploader"] {
        width: 100%;
    }
    ...
    .st-key-landing_upload section {
        ...
        border: 1.5px solid #83cdbd !important;
        border-radius: 16px !important;
        background: rgba(255,255,255,.92) !important;
        box-shadow: 0 7px 24px rgba(30, 90, 76, .055);
    }
    ...
    .st-key-sample_download .stDownloadButton > button {
        min-height: 52px;
        border-radius: 14px;
        border: 1px solid rgba(35, 42, 40, .30);
        background: white;
        font-weight: 650;
    ...
    @media (max-width: 900px) {
        ...
        .landing-spacer {height: 2rem;}
        .landing-title-card {padding: 1rem 1.2rem;}
    }
</style>
"""
...
def render_upload_screen() -> None:
    ...
    with center:
        ...
        if SAMPLE_PATH.exists():
            st.markdown('<div class="landing-sample-label">or try the sample model</div>', unsafe_allow_html=True)
