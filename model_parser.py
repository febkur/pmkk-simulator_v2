...
def _directorate(comment: str) -> str | None:
    """Read and normalize a directorate tag from the Vensim comment.

    Recommended syntax:
        @output directorate=NAKER
    Long names are also accepted, e.g.:
        @output directorate="Ketenagakerjaan"
    """
    raw = (
        _annotation_value(comment, "directorate")
        or _annotation_value(comment, "direktorat")
        or _annotation_value(comment, "group")
    )
    if not raw:
        return None
    ...
    return DIRECTORATE_ALIASES.get(normalized, raw.strip())


def _indicator(comment: str) -> str | None:
    """Read an official PMKK indicator number from a Vensim comment."""
    raw = _annotation_value(comment, "indicator")
    if not raw:
        return None
...
def parse_model_text(text: str, filename: str = "model.mdl") -> ParsedModel:
    ...
    if tagged_inputs:
        # Explicit tags are authoritative: every tagged input is exposed.
        input_vars = tagged_inputs
    else:
        # Untagged models use a conservative fallback so a large technical model
        # does not accidentally expose hundreds of constants as policy levers.
        input_vars = [
            variable
    ...
    for variable in input_vars:
        ...
        inputs.append(
            VariableMeta(
                ...
                max=max_value,
                step=step_value,
                directorate=_directorate(variable["comment"]),
                indicator=_indicator(variable["comment"]),
            )
        )

    tagged_outputs = [variable for variable in variables if variable["tag_output"]]
    if tagged_outputs:
        # Explicit tags are authoritative: the graph grid adapts to the count.
        output_vars = tagged_outputs
    else:
        stocks = [variable for variable in variables if variable["is_stock"]]
        output_vars = stocks[:8]
        if not output_vars:
